<?php
/**
 * The template part for displaying Content of topic.
 *
...
if ( $terms && ! is_wp_error( $terms ) ) :
	...
	$all_terms  = '<div class="flex flex-wrap gap-1">' . $join_terms . '</div>';
...
if ( $terms_tag && ! is_wp_error( $terms_tag ) ) :
	...
	$tag_terms      = '<div class="flex flex-wrap gap-1">' . $join_tag_terms . '</div>';
endif;
...
printf(
	'<article class="grid bg-white shadow-course rounded-10">
		...
		<div class="grid p-6">
			...
			<div><a href="%s" class="button button-medium mt-6">%s</a></div>
		</div>
	</article>',
